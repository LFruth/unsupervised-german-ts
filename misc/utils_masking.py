...
class NonStopMasker(Masker):
    def __init__(self):
        ...
        self.stop_ws = set(nltk.corpus.stopwords.words("german") + [",", "''", "``", "--", "-", ".", "(", ")", ";", ":"])
        self.mask_id = 5
    ...
    def mask(self, sentences):
        ...
        for sentence in sentences:
            ...
            for w in words:
                ...
                ums += toks
                even += 1
                if w.lower() not in self.stop_ws and len(w) > 2 and even % 2 == 0:
                    ms += [self.mask_id] * len(toks)
                    ims += [1] * len(toks)
                else:
                    ms += toks
                    ims += [0] * len(toks)
